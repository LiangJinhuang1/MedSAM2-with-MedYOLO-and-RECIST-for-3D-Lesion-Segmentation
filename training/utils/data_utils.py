...
class BatchedVideoMetaData:
    ...
    Attributes:
        ...
        spacing:                   Tensor of shape [B,3]  voxel spacing (sz,sy,sx) per video.
        direction:                 Tensor of shape [B,9]  row-major 3x3 direction cosines per video.
        origin:                    Tensor of shape [B,3]  world origin (oz,oy,ox) per video.
    """
    
    unique_objects_identifier: torch.LongTensor
    frame_orig_size: torch.LongTensor
    spacing: torch.FloatTensor
...
class VideoDatapoint:
    ...
    markers: Optional[List[List[torch.Tensor]]] = None  # [frames][objects] or your chosen structure
...
def collate_fn(
    ...
    # obj_to_frame_idx: [T, O, 2] (frame, video)
    video_indices = obj_to_frame_idx[..., 1]  # [T, O]

    # Now, for each object, get the corresponding video-level metadata
    spcs = torch.stack([
        torch.from_numpy(video.metadata["spacing"]).float()
        for video in batch
    ], dim=0)  # [B, 3]
    dirs = torch.stack([
        torch.from_numpy(video.metadata["direction"]).float()
        for video in batch
    ], dim=0)  # [B, 9]
    orgs = torch.stack([
        torch.from_numpy(video.metadata["origin"]).float()
        for video in batch
    ], dim=0)  # [B, 3]

    # Now, expand to [T, O, ...] using video_indices
    spcs_per_obj = spcs[video_indices]  # [T, O, 3]
    dirs_per_obj = dirs[video_indices]  # [T, O, 9]
    orgs_per_obj = orgs[video_indices]  # [T, O, 3]
    ...
    return BatchedVideoDatapoint(
        ...
        metadata=BatchedVideoMetaData(
            ...
            spacing=spcs_per_obj,
            direction=dirs_per_obj,
            origin=orgs_per_obj
        ),
        dict_key=dict_key,
        batch_size=[T],
