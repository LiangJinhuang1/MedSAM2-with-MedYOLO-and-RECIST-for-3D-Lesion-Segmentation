...
class SAM2Train(SAM2Base):
    def __init__(
        ...
        **kwargs,
        ...
        self.use_racist_marker = use_racist_marker
    ...
    def forward(self, input: BatchedVideoDatapoint):
        ...
        # Temporarily disabled due to tensor count mismatch issues
        if False and self.training:
            previous_stages_out = torch.utils.checkpoint.checkpoint(
    ...
    def prepare_prompt_inputs(self, backbone_out, input, start_frame_idx=0):
        ...
        for t in init_cond_frames:
            if not use_pt_input:
                ...
                # Standard training prompt generation
                ...
                if use_box_input:
                    ...
                    if self.use_racist_marker:
                        gt2d = gt_masks_per_frame[t]       # tensor [B,1,H,W], bool
                        B, _, H, W = gt2d.shape
                        points = torch.zeros((B, 4, 2), device=self.device, dtype=torch.float)
                        labels = torch.ones((B, 4), device=self.device, dtype=torch.int32)
                        
                        for b in range(B):
                            arr = gt2d[b,0].cpu().numpy().astype(bool)
                            if np.sum(arr) == 0:
                                continue
                                
                            # Downsample mask for faster computation
                            downsample_factor = 4  # Try 4 or 8 for big masks
                            arr_small = cv2.resize(arr.astype(np.uint8), (arr.shape[1]//downsample_factor, arr.shape[0]//downsample_factor), interpolation=cv2.INTER_NEAREST)
                            ys, xs = np.where(arr_small)
                            if len(xs) < 2:
                                continue
                            mask_points = np.stack([xs, ys], axis=1)  # (x, y) format in downsampled space
                            
                            # Find longest diameter endpoints
                            dist_matrix = squareform(pdist(mask_points))
                            i, j = np.unravel_index(np.argmax(dist_matrix), dist_matrix.shape)
                            p1_long = mask_points[i]
                            p2_long = mask_points[j]
                            
                            # Find perpendicular diameter endpoints
                            long_vec = p2_long - p1_long
                            long_vec = long_vec / np.linalg.norm(long_vec)
                            perp_vec = np.array([-long_vec[1], long_vec[0]])
                            
                            perp_distances = np.dot(mask_points - p1_long, perp_vec)
                            perp_min_idx = np.argmin(perp_distances)
                            perp_max_idx = np.argmax(perp_distances)
                            p1_perp = mask_points[perp_min_idx]
                            p2_perp = mask_points[perp_max_idx]
                            
                            # Map back to original coordinates
                            p1_long_orig = p1_long * downsample_factor
                            p2_long_orig = p2_long * downsample_factor
                            p1_perp_orig = p1_perp * downsample_factor
                            p2_perp_orig = p2_perp * downsample_factor
                            
                            # Store 4 RECIST endpoints
                            points[b, 0] = torch.tensor(p1_long_orig, dtype=torch.float, device=self.device)
                            points[b, 1] = torch.tensor(p2_long_orig, dtype=torch.float, device=self.device)
                            points[b, 2] = torch.tensor(p1_perp_orig, dtype=torch.float, device=self.device)
                            points[b, 3] = torch.tensor(p2_perp_orig, dtype=torch.float, device=self.device)
    ...
    def _iter_correct_pt_sampling(
        ...
        for _ in range(self.num_correction_pt_per_frame):
            ...
            # Temporarily disabled gradient checkpointing due to tensor count mismatch issues
            if False and self.use_act_ckpt_iterative_pt_sampling and not multimask_output:
