...
def single_proc_run(local_rank, main_port, cfg, world_size, node_rank, master_addr):
    ...
    try:
        ...
        if hasattr(trainer, '_cleanup_distributed'):
...
def main(args, cfg) -> None:
    ...
    print("=" * 50)
    print(f"  📦 Box Input Probability: {getattr(cfg.trainer.model, 'prob_to_use_box_input_for_train', 0.0)}")
    print(f"  📍 Point Input Probability: {getattr(cfg.trainer.model, 'prob_to_use_pt_input_for_train', 0.0)}")
    print(f"  📐 RECIST Marker: {getattr(cfg.trainer.model, 'use_racist_marker', False)}")
    print(f"  🎲 Enhanced Strategy Prob: {getattr(cfg.trainer.model, 'prob_enhanced_strategy', 0.0)}")
    print("=" * 50)
    ...
    training_info = {
        "training_type": "diameter_bbox_optimized",
        "key_features": {
            "box_input_probability": getattr(cfg.trainer.model, 'prob_to_use_box_input_for_train', 0.0),
            "point_input_probability": getattr(cfg.trainer.model, 'prob_to_use_pt_input_for_train', 0.0),
            "use_racist_marker": getattr(cfg.trainer.model, 'use_racist_marker', False),
            "enhanced_strategy_prob": getattr(cfg.trainer.model, 'prob_enhanced_strategy', 0.0),
        },
        "training_objectives": [
            "Optimize for diameter-based bounding box prompts",
            "Improve 3D segmentation from 2D diameter markers",
            "Better pan-cancer foundation model performance",
            "Higher DSC scores with bounding box prompts"
        ]
    }
    
    with g_pathmgr.open(
        os.path.join(cfg.launcher.experiment_log_dir, "training_info.yaml"), "w"
    ) as f:
        f.write(OmegaConf.to_yaml(training_info))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt-path", type=str, default=None, help="Path to the pretrained checkpoint"
    )
    parser.add_argument(
        "--mode", type=str, default="train", help="Mode to run the training, train or finetune"
    )
    ...
    # Diameter bounding box training arguments
    parser.add_argument("--box-prob", type=float, default=None, help="Override box input probability (0.0-1.0)")
    parser.add_argument("--point-prob", type=float, default=None, help="Override point input probability (0.0-1.0)")
    parser.add_argument("--enable-racist", action="store_true", help="Enable RECIST marker integration")
    ...
    if args.dataset_path is not None:
        cfg.trainer.data.train.datasets[0].dataset.datasets[0].video_dataset.folder = args.dataset_path
    if args.output_path is not None:
        cfg.launcher.experiment_log_dir = args.output_path
    
    if args.ckpt_path is not None:
        cfg.trainer.resume_from = args.ckpt_path
    
    if args.mode == "finetune":
        cfg.trainer.mode = "finetune"
    
    # Handle diameter bounding box training arguments
    if args.box_prob is not None:
        if 0.0 <= args.box_prob <= 1.0:
            cfg.trainer.model.prob_to_use_box_input_for_train = args.box_prob
            print(f"🎯 Box input probability set to: {args.box_prob}")
        else:
            print(f"⚠️  Invalid box probability: {args.box_prob}. Must be between 0.0 and 1.0")
    
    if args.point_prob is not None:
        if 0.0 <= args.point_prob <= 1.0:
            cfg.trainer.model.prob_to_use_pt_input_for_train = args.point_prob
            print(f"🎯 Point input probability set to: {args.point_prob}")
        else:
            print(f"⚠️  Invalid point probability: {args.point_prob}. Must be between 0.0 and 1.0")
    
    if args.enable_racist:
        cfg.trainer.model.use_racist_marker = True
        print("🎯 RECIST marker integration enabled")
