...
class Trainer:
    ...
    def _cleanup_distributed(self):
        """Clean up distributed training resources to prevent NCCL warnings."""
        try:
            if hasattr(self, 'distributed_rank') and self.distributed_rank == 0:
                if dist.is_initialized():
                    logging.info("Cleaning up distributed process group...")
    ...
    def __del__(self):
        """Destructor to ensure cleanup even if the object is garbage collected."""
        try:
            self._cleanup_distributed()
        except:
    ...
    def train_epoch(self, train_loader):
        ...
        for data_iter, batch in enumerate(train_loader):
            ...
            try:
                ...
                # More frequent cache clearing for memory optimization
                if data_iter % 10 == 0:  # Reduced from 20 to 10
                    torch.cuda.empty_cache()
                    gc.collect()
            ...
            # Catching NaN/Inf errors in the loss
                ...
                if "out of memory" in str(e).lower():
                    logging.error(f"OOM error at iteration {data_iter}: {e}")
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                        gc.collect()
                    logging.error("Memory cleared. Consider reducing batch size further.")
                    raise
                else:
                    raise
