...
class NPZRawDataset(VOSRawDataset):
    ...
    def get_video(self, idx):
        ...
        masks = npz_data['gts']
        spacing, direction, origin = npz_data['spacing'], npz_data['direction'], npz_data['origin']
        
        if self.truncate_video > 0:
            frames = frames[:self.truncate_video]
            masks = masks[:self.truncate_video]
            spacing=spacing[:self.truncate_video]
            direction=direction[:self.truncate_video]
            origin=origin[:self.truncate_video]
        ...
        video.metadata = {'spacing': spacing, 'direction': direction, 'origin': origin}
        return video, segment_loader
