...
def nifti_check_anchors(dataset, model, thr=4.0, imgsz=default_size):
    ...
    def metric(k):  # compute metrics for anchors
        # k should have shape (num_anchors, 3)
        # dwh has shape (num_labels, 3)
        # We want to compute ratio between each label and each anchor
        r = dwh[:, None, :] / k[None, :, :]  # shape: (num_labels, num_anchors, 3)
        x = torch.min(r, 1./r).min(2)[0] # ratio metric, shape: (num_labels, num_anchors)
        best = x.max(1)[0]  # best_x, shape: (num_labels,)
    ...
    if bpr < 0.98:  # threshold to recompute
        ...
        # anchors is a numpy array with shape (3, 6, 3) - need to flatten to (18, 3)
        anchors_tensor = torch.tensor(anchors, dtype=torch.float32).view(-1, 3)
        new_bpr = metric(anchors_tensor)[0]
