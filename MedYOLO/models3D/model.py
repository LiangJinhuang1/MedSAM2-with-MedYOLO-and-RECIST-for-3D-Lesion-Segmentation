...
def attempt_load(weights, map_location=None, inplace=True, fuse=True):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        # Handle MPS float64 compatibility issue during loading
        if map_location and str(map_location).startswith('mps'):
            # For MPS, load to CPU first to avoid float64 issues, then move to MPS
            try:
                # Try loading with weights_only=True first (PyTorch 2.6+ default)
                ckpt = torch.load(str(Path(str(w).strip().replace("'", ''))), map_location='cpu', weights_only=True)
            except Exception as e:
                # If that fails, try with weights_only=False (legacy behavior)
                print(f"Warning: Loading checkpoint with weights_only=False due to: {e}")
                ckpt = torch.load(str(Path(str(w).strip().replace("'", ''))), map_location='cpu', weights_only=False)
            
            # Convert any float64 tensors to float32 before moving to MPS
            loaded_model = ckpt['ema' if ckpt.get('ema') else 'model']
            ...
            loaded_model = loaded_model.to(map_location)
        else:
            # For non-MPS devices, load normally
            try:
                # Try loading with weights_only=True first (PyTorch 2.6+ default)
                ckpt = torch.load(str(Path(str(w).strip().replace("'", ''))), map_location=map_location, weights_only=True)
            except Exception as e:
                # If that fails, try with weights_only=False (legacy behavior)
                print(f"Warning: Loading checkpoint with weights_only=False due to: {e}")
                ckpt = torch.load(str(Path(str(w).strip().replace("'", ''))), map_location=map_location, weights_only=False)
            
            loaded_model = ckpt['ema' if ckpt.get('ema') else 'model']
        
        if fuse:
            model.append(loaded_model.float().fuse().eval())  # FP32 model
        else:
            model.append(loaded_model.float().eval())  # without layer fuse
...
class SPPF(nn.Module):
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            # Handle MPS compatibility for max_pool3d
            if x.device.type == 'mps':
                # For MPS, move to CPU for max_pool3d operations, then back to MPS
                x_cpu = x.cpu()
                y1 = self.m(x_cpu).to(x.device)
                y2 = self.m(y1.cpu()).to(x.device)
                y3 = self.m(y2.cpu()).to(x.device)
            else:
                # For other devices, use normal operations
                y1 = self.m(x)
                y2 = self.m(y1)
                y3 = self.m(y2)
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):           
            ...
            if not self.training: # inference
                ...
                y = x[i].sigmoid()
                # Ensure grid tensors are on the same device as input tensor
                grid_i = self.grid[i].to(x[i].device)
                anchor_grid_i = self.anchor_grid[i].to(x[i].device)
                stride_i = self.stride[i].to(x[i].device)
                
                # in training mode these are calculated in the loss function
                y[..., 0:3] = (y[..., 0:3] * 2. - 0.5 + grid_i) * stride_i  # zxy
                y[..., 3:6] = (y[..., 3:6] * 2) ** 2 * anchor_grid_i  # dwh
